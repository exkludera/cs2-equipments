...
public static class Item_Backpack
{
    private static Dictionary<ulong, CBaseModelEntity> Equipment = new();

    public static void OnPluginStart()
    {
        Item.RegisterType("backpack", OnServerPrecacheResources, OnEquip, OnUnequip, true, null);
        Instance.RegisterEventHandler<EventPlayerSpawn>(OnPlayerSpawn);
    }
    public static void OnServerPrecacheResources(ResourceManifest manifest)
    {
        List<KeyValuePair<string, Dictionary<string, string>>> items = Item.GetItemsByType("backpack");
